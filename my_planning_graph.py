    ...
    def add_action_level(self, level):
        ...
        for state in self.s_levels[level]:
            for action in self.all_actions:
                if (state.is_pos and state.symbol in action.precond_pos) or (not state.is_pos and state.symbol in action.precond_neg):
    ...
    def h_levelsum(self) -> int:
        ...
        for goal_literal in self.problem.goal:
            for level in range(0, len(self.s_levels)):
                nodes = [n.symbol for n in self.s_levels[level] if n.is_pos]
                if goal_literal in nodes:
